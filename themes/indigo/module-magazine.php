...
$pagination 	= esc_attr( get_post_meta( $mod_id, 'quadro_mod_magazine_pag', true ) );
$chosen_perpage = $chosen_perpage != '' ? $chosen_perpage : get_option( 'posts_per_page' );
...
// Set pagination parameters if set
if ( ( $layout == 'layout4' || $layout == 'layout3' ) && $pagination !== false && $pagination == 'enabled' ) {
	global $paged;
	$page_var = is_front_page() ? 'page' : 'paged';
	$paged = (get_query_var($page_var)) ? get_query_var($page_var) : 1;
	$offset = $paged == 1 ? $offset : ( $offset + ( $paged - 1 ) * $items_perpage );
}
...
// Add pagination parameters if set
if ( ( $layout == 'layout4' || $layout == 'layout3' ) && $pagination !== false && $pagination == 'enabled' ) {
	$args['paged'] = $paged;
...
<section id="post-<?php the_ID(); ?>" class="quadro-mod type-magazine magazine-<?php echo $layout; ?> mag-columns-<?php echo $columns; ?> <?php quadro_mod_parallax($mod_id); ?> modheader-<?php echo $header_layout; ?> overlay-<?php echo $overlay; ?>">
	...
	<div class="mod-content">
		...
		<div class="inner-mod">
						...
						<article class="clear">
							...
							<div class="entry-thumbnail">
								<?php if ( $post_format != 'video' || has_post_thumbnail() ) { ?>
									...
									$back_img = 'style="background-image: url(\'' . esc_url($image[0]) . '\');"'; ?>
									<a href="<?php the_permalink(); ?>" rel="bookmark" class="lazy" data-original="<?php echo esc_url($image[0]); ?>" <?php echo $back_img; ?> ></a>
								<?php } else {
							...
							<div class="magazine-content">
								...
								<?php if ( get_the_title() == '' && ($post_format == 'aside' || $post_format == 'status' || $post_format == 'quote') ) { ?>
									
									<h1 class="entry-title">
										<a href="<?php the_permalink(); ?>" rel="bookmark">
											<?php echo quadro_excerpt(get_the_excerpt(), 10, ''); ?>
										</a>
									</h1>
								...
								if ( ($layout == 'layout1' && $i == 1) || $layout == 'layout3' || ($layout == 'layout2' && ($i == 1 || $i == 2)) || ($layout == 'layout2' && $columns == 'three' && $i == 3) || ($layout == 'layout4' && $excerpt == 'show') ) { ?>
			...
			<?php if ( ( $layout == 'layout4' || $layout == 'layout3' ) && $pagination !== false && $pagination == 'enabled' ) quadro_paging_nav( '<i class="fa fa-angle-left"></i>', '<i class="fa fa-angle-right"></i>' ); ?>
