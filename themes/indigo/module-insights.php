				...
				<?php if ( is_array($insights) ) {
					foreach ($insights as $insight) { ?>
						<li class="insight <?php echo esc_attr( $insight['layout'] ); ?>-insight clear">
							...
							<?php // Check for title and content and output them
							if (  $insight['title'] != '' || $insight['content'] != '' ) { ?>
								<div class="insight-content <?php echo $qi_animations[$anim]['second']; ?>" data-wow-delay="<?php echo $anim_delay; ?>ms" style="color: <?php echo $color; ?>;">
									<?php if ( $insight['title'] != '' ) {
										echo '<h4 class="insight-title">' . esc_attr( $insight['title'] ) . '</h4>';
									}
									if ( $insight['content'] != '' ) {
										$content = wp_kses_post( $insight['content'] );
										echo '<div class="insight-text">';
										echo apply_filters( 'the_content', $content );
										echo '</div>';
									}
									if ( $insight['button_text'] != '' ) { ?>
										<a href="<?php echo esc_url($insight['button_url']); ?>" title="<?php echo esc_attr($insight['button_text']); ?>" class="qbtn insight-link scroll-to-link" style="color: <?php echo $color; ?>;"><?php echo esc_attr($insight['button_text']); ?></a>
									<?php } ?>
								</div>
							<?php } ?>
