			...
			<ul class="authors-list">
				<?php if ( is_array($authors) ) : foreach ( $authors as $author ) : ?>
					...
					<li>
						...
						$style = $cover_img != '' ? 'style="background-image: url(' . esc_url( $cover_img ) . ');"' : '';
						?>
						...
						<div class="author-name">
							<?php if ( get_avatar( $author->user_email ) ) {
								echo '<a href="' . esc_url( $author_link ) . '" title="' . esc_html__('Posts by ', 'indigo') . esc_attr($author->display_name) . '">';
								echo get_avatar( $author->user_email, $size='120', '', esc_attr($author->display_name) ) . '</a>'; 
							} ?>
							<h3>
								<a href="<?php echo esc_url( $author_link ); ?>" title="<?php echo esc_html__('Posts by ', 'indigo') . esc_attr($author->display_name); ?>">
						...
						<?php if ( $authors_bio == 'show' ) {
							echo '<div class="author-bio">' . wp_kses_post( get_the_author_meta( 'description', $author->ID ) ) . '</div>';
						} ?>
								...
								// Now get methods defined at Theme Options >> Social Tab
				 				...
				 				if ( is_array( $quadro_options['available_contact_methods'] ) ) {
				 					foreach ( $quadro_options['available_contact_methods'] as $method ) {
										...
										if ( $contact != '' )
											echo '<a class="author-' . $method['slug'] . '" href="' . $contact . '" title="' . esc_html__('Follow', 'indigo') . ' ' . $author->display_name . ' ' . esc_html__('on', 'indigo') . ' ' . $method['title'] . '"><i class="fa fa-' . $method['slug'] . '"></i></a>';
				 					}
				 				} ?>
							</div>
						<?php } ?>
