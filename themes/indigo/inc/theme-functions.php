...
if ( ! function_exists( 'qi_body_class' ) ) :
function qi_body_class( $classes ) {
	
	global $quadro_options;

	// Get ID (with conditional for WooCommerce shop)
	...
	$add_classes = array( 	'site-' . $site_layout, 
							$header_style . '-header',
							$header_layout,
							$header_height . '-header',
							$transp_color . '-header',
							$sticky_header . '-header',
							$show_sidebar . '-' . $sidebar_pos,
							'woo-sidebar-' . $quadro_options['woo_sidebar'],
							'woo-cols' . $quadro_options['woo_loop_columns'],
							'icons-' . $quadro_options['icon_font'] 
					);
...
add_filter( 'body_class', 'qi_body_class' );
...
function quadro_more_tag() {
	return '<a href="' . esc_url( get_the_permalink() ) . '" class="readmore-link"><span class="read-more">' . esc_html__('Read more', 'indigo') . '</span></a>';
}
endif; // if !function_exists
...
function quadro_site_menu() {
	// escape early if menu is hidden
	global $quadro_options;
 ...
 */
if ( ! function_exists( 'quadro_hamburger_menu' ) ) :
...
function quadro_header_login() {
	...
	<?php } else { 
		...
		<a href="<?php echo esc_url( $quadro_options['header_logged_url'] ); ?>" title="<?php echo esc_attr( $current_user->display_name ); ?>">
			<i class="fa fa-user"></i><span><?php echo esc_attr( $current_user->display_name ); ?></span></a>
		...
		<div class="inner-author">
			...
			<div class="author-name">
				<?php if ( get_avatar( $author->user_email ) ) {
					echo '<a href="' . $author_link . '" title="' . esc_html__('Posts by ', 'indigo') . $author->display_name . '">';
					echo get_avatar( $author->user_email, $size='120', '', esc_attr($author->display_name) ) . '</a>'; 
				} ?>
				<h3>
					<a href="<?php echo $author_link; ?>" title="<?php echo esc_html__('Posts by ', 'indigo') . esc_attr($author->display_name); ?>">
				...
				// Now get methods defined at Theme Options >> Social Tab
 				if ( is_array( $quadro_options['available_contact_methods'] ) ) {
 					foreach ( $quadro_options['available_contact_methods'] as $method ) {
						...
						if ( $contact != '' )
							echo '<a class="author-' . $method['slug'] . '" href="' . $contact . '" title="' . esc_html__('Follow', 'indigo') . ' ' . esc_attr( $author->display_name ) . ' ' . esc_html__('on', 'indigo') . ' ' . $method['title'] . '"><i class="fa fa-' . $method['slug'] . '"></i></a>';
 					}
 				} ?>
			</div>
...
$header_layouts = array(
	...
	'header-layout5' => array(
		'1st' => array(
			'left' => array( 'quadro_site_title' ),
			'center' => array(),
			'right' => array( 'quadro_site_menu', 'quadro_social_icons_header', 'qi_header_extras', 'quadro_hamburger_menu' )
		),
		'2nd' => array(
			'left' => array(),
			'center' => array(),
...
// Apply Filter For Header Layouts Definitions
$header_layouts = apply_filters( 'qi_filter_header_layouts', $header_layouts );
	...
	if ( $header_layout != 'none' ) {
		echo '<header class="mod-header">';
			echo '<div class="inner-mod">';
				...
				if ( $header_btn != "" ) echo '<a href="' . $header_btn_url . '" class="modheader-btn qbtn">' . $header_btn . '</a>';
			echo '</div><!-- .inner-mod -->';
...
function quadro_show_user_cover_img( $user ) { ?>
	<h3><?php esc_html_e('Author Page Cover Image', 'indigo'); ?></h3>
	<table class="form-table">
		<tr>
			<th><label for="twitter"><?php esc_html_e('Image', 'indigo'); ?></label></th>
			<td>
				...
				<input class="upload_file_button" type="button" value="<?php esc_html_e( 'Select file', 'indigo' ) ?>" />
				<p><?php esc_html_e( 'Once you select the file, click "Choose Image".', 'indigo' ); ?></p>
			</td>
		</tr>
	</table>
<?php }
...
function quadro_save_user_cover_img( $user_id ) {
	...
	if ( !current_user_can( 'edit_user', $user_id ) || !isset( $_POST['cover_img'] ) )
		return false;
...
function quadro_single_portfolio_item( $item_id ) {
	...
	$feat_img_pos 	= esc_attr( get_post_meta( $item_id, 'quadro_portfolio_feat_image', true ) );
	...
	// Prepare Feat. Image Style
	$feat_style 	= $feat_img_pos == 'header' ? 'style="background-image: url(\'' . wp_get_attachment_url( get_post_thumbnail_id($item_id) ) . '\');"' : '';

	?>

	<article id="post-<?php the_ID(); ?>" <?php post_class( 'clear ' . $item_style . ' feat-img-' . $feat_img_pos ); ?>>

		<header class="item-header" <?php echo $feat_style; ?>>
			...
			<div class="actions-header">
				<a href="<?php echo esc_url( get_permalink( $prev_id ) ); ?>" class="item-link prev-portf-item <?php echo $disabled_prev; ?>" title="<?php esc_html_e('Prev Item', 'indigo') ?>" data-id="<?php echo $prev_id; ?>"><span class="prev-item"><i class="fa fa-angle-left"></i></span></a>
				<a href="<?php echo esc_url( get_permalink( $next_id ) ); ?>" class="item-link next-portf-item <?php echo $disabled_next; ?>" title="<?php esc_html_e('Next Item', 'indigo') ?>" data-id="<?php echo $next_id; ?>"><span class="next-item"><i class="fa fa-angle-right"></i></span></a>
			</div>
		...
		<div class="item-body clear">
			...
			<?php if ( !isset( $quadro_options['portf_transients_enable'] ) || $quadro_options['portf_transients_enable'] == 'enabled' ) {
				...
				// Set both to false to prevent caching
				$content_transient = false;
				$media_transient = false;
			}
			...
			<?php if ( ( false === $content_transient || false === $media_transient ) && ( !isset( $quadro_options['portf_transients_enable'] ) || $quadro_options['portf_transients_enable'] == 'enabled' ) ) {
			...
			} else if ( !isset( $quadro_options['portf_transients_enable'] ) || $quadro_options['portf_transients_enable'] == 'enabled' ) {
...
function quadro_get_portfolio_fields($item_id) {
	...
	if ( $print_fields != '' || $ext_link != '' ) {
		echo '<div class="item-data clear">';
