...
function quadro_breadcrumbs() {
	...
	if ( is_category() || is_single() ) {
		// Escape this for WooCommerce and bbPress
...
function quadro_excerpt( $excerpt = '', $excerpt_length = 20, $readmore = '', $tags = '' ) {
	...
	if (strlen($excerpt) > $excerpt_length) {
		$temp_content = strip_tags( substr($excerpt, 0, $excerpt_length), $tags );
...
/**
 * Quadro Custom get_the_excerpt and changes the "more" tag
 */
if ( ! function_exists( 'get_quadro_excerpt' ) ) :
function get_quadro_excerpt($post, $excerpt = '', $excerpt_length = 20, $readmore = '', $tags = '' ) {
 ...
 */
function quadro_maybe_print_gallery_field( $field, $id, $size = 'full', $class = 'gallery-field', $caption = false, $link = false ) {
	...
	foreach( $pic_ids[0] as $pic_id ) {
		...
		if ( $caption == true ) echo '<p class="gallery-caption">' . esc_attr( get_post( $pic_id )->post_excerpt ) . '</p>';
		if ( $link == true ) {
			$image = wp_get_attachment_image_src( $pic_id, 'full' );
			echo '<a href="' . esc_url( $image[0] ) . '" rel="lightbox"></a>';
		}
		echo '</li>';
	}
	echo '</ul></div>';
...
function quadro_maybe_print_gallery_field_as_bg( $field, $id, $size = 'full', $class = 'gallery-field' ) {
	...
	foreach( $pic_ids[0] as $pic_id ) {
		...
		echo '<li style="background-image: url(\'' . esc_url($image[0]) . '\');"></li>';
	}
	echo '</ul></div>';
...
function quadro_video_screenshot($content, $permalink, $title) {
	...
	foreach ($urls as $url) {
		...
		if ( strpos( $url, 'vimeo' ) !== false ) {
			...
			$hash = unserialize( wp_remote_fopen('https://vimeo.com/api/v2/video/' . $video_id . '.php') );
...
/**
 * Returns a lighter or darker version of a hex color
 * http://stackoverflow.com/questions/3512311/how-to-generate-lighter-darker-color-with-php
 */
function qi_adjust_brightness($hex, $steps) {
