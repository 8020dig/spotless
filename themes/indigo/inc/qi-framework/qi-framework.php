 ...
 * $qi_docs_url 			- Sets the URL for this theme's docs
 * $qi_support_url			- Sets theme support URL for internal links
 * $dcontent_file		- Sets RMS file name if it'll be use for RMS import
 * $dcontent_settings	- Sets RMS settings file name if it'll be use for RMS import
 * $dcontent_widgets	- Sets RMS widgets file name if it'll be use for RMS import
 * $dcontent_plugins	- Sets joined plugins array to be used for RMS import
 * $dcontent_reading	- Sets Reading Settings option to be used for RMS import
 * $dcontent_front		- Sets Reading Settings Front Page option to be used for RMS import
 * $dcontent_posts		- Sets Reading Settings Posts Page option to be used for RMS import
 */
...
if ( is_admin() ) {
	...
	/**
	 * Including Theme Licensing Check (ADMIN ONLY)
	 */
	if ( isset( $quadro_options['quadro_username'] ) && $quadro_options['quadro_username'] !== false ) {
		require( $template_directory . '/inc/qi-framework/license-check.php' );
	}
...
function qi_scripts() {
	...
	if ( !isset($quadro_options['icon_font']) || $quadro_options['icon_font'] == 'font-awesome' ) {
		...
		wp_enqueue_style( 'font-awesome', $template_directory_uri . '/inc/qi-framework/fonts/font-awesome/font-awesome.min.css' );
	} else {
...
function qi_admin_scripts() {
	...
	global $template_directory_uri, $quadro_options, $theme_slug;
	...
	wp_localize_script( 'adminscripts', 'adminscripts_localized',
		array(
			...
			'importConfirm'	=> esc_html__( 'Click OK to import this Ready Made Site into your WordPress install. Several WordPress options and all your theme options will be replaced!', 'quadro' ),
			...
			'importSuccess'	=> esc_html__( 'Ready Made Site successfully imported. Page needs to be refreshed.', 'quadro' ),
...
function qi_script_tag_defer( $tag, $handle ) {
	...
	// return tag always if option disabled
	if ( $quadro_options['scripts_deferring'] === 'disabled' )
		return $tag;
...
/*
 * Removes the "View" action from  Modules list on backend (we use our
 * own "Preview" action for that)
 */
function qi_remove_mods_view( $actions, $post ) {
	if ( get_post_type() === 'quadro_mods' ) {
		unset( $actions['view'] );
	}
	return $actions;
}
add_filter( 'post_row_actions', 'qi_remove_mods_view', 10, 2 );
