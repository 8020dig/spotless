...
/**
 * Function that imports selected RMS (with options & widgets)
 * into a WordPress install.
 * 
 * Needs to be passed the selected RMS as an argument.
 */
function quadro_dcontent_importx( $pack ) {
	
	global $wpdb, $quadro_options_group, $quadro_options;

	// Bring Available Ready Made Sites
	...
	if ( current_user_can('manage_options') ) {
		...
		if ( class_exists( 'WP_Importer' ) && class_exists( 'WP_Import' ) ) {
			...
			if ( !is_null($homepage) && $homepage->ID ) {
				update_option( 'page_on_front', $homepage->ID );
			}

			if ( !is_null($posts_page) && $posts_page->ID ) {
