...
function quadro_print_boxes( $quadro_current_fields, $post_id = '' ) {
	...
	echo '<table>';
		...
		switch ( $field['type'] ) {
			...
			case 'number':
				echo '<input type="number" name="', $field['id'], '" id="', $field['id'], '" value="', $meta ? $meta : '', '" size="30" style="width: 60px; margin-right: 10px;" />';
			...
			case 'prev-editor':
				...
				<small><?php esc_html_e( 'When entering HTML content, please update module twice for the editor to store it.', 'quadro' ); ?></small> <?php
				break;
			...
			case 'posts_picker' :
				...
				if( !empty($select_query) ) :
					...
					if ( $field['post_type'] == 'quadro_mods' ) {
						...
						foreach ($qi_available_modules as $key => $value) {
			...
			case 'pattern_picker':
				...
				foreach ( $quadro_patterns as $quadro_pattern ) {
					echo '<label><input type="radio" name="', $field['id'], '"' . checked( $meta == $quadro_pattern ? true : false ) . ' value="' . $quadro_pattern . '" /><img src="' . get_stylesheet_directory_uri() . '/images/admin/pattern-thumbs/' . $quadro_pattern . 'thumb.jpg" width="30px" height="30px" /></label>';
				}
				echo '</div>';
				break;
			...
			case 'tax_picker' :
				...
				$terms = get_terms( array( 'taxonomy' => $field['tax_slug'], 'hide_empty' => 0 ) );
				// List them
				if ( count($terms) > 0 ) :
							...
							foreach ( $field['repeat-fields'] as $this_field ) {
								...
								switch ( $this_field['type'] ) {
									case 'text':
										...
										if ( $this_field['desc'] != '' ) {
											echo '<p class="description">' . $this_field['desc'] . '</p>';
										}
										break;
									case 'checkbox':
										$this_value = (!empty($row[$this_field['name']])) ? esc_attr($row[$this_field['name']]) : '';
										echo '<input type="hidden" name="' . $field['id'] . '['.$i.']['. $this_field['name'] .']" value=false />';
										echo '<input type="checkbox" name="' . $field['id'] . '['.$i.']['. $this_field['name'] .']" id="', $field['id'], '" value=true ', $this_value == 'true' ? ' checked="checked"' : '', ' />';
										break;
									case 'color':
										$this_value = (!empty($row[$this_field['name']])) ? esc_attr($row[$this_field['name']]) : '#';
										echo '<input type="text" class="repeat-color" name="' . $field['id'] . '['.$i.']['. $this_field['name'] .']" id="', $field['id'], '" value="' . $this_value . '" size="15"/>
										<input type="button" class="quadropickcolor button-secondary" value="' . esc_html__( 'Select color', 'quadro' ) . '">
										<div id="qcolorpicker"></div>';
										break;
									case 'textarea':
										$this_value = (!empty($row[$this_field['name']])) ? esc_attr($row[$this_field['name']]) : '';
										if ( isset( $this_field['desc'] ) && $this_field['desc'] != '' ) {
											echo '<p class="description">' . $this_field['desc'] . '</p>';
										}
									...
									case 'social':
										...
										echo '<label>Instagram: </label><input type="text" name="' . $field['id'] . '['.$i.']['. $this_field['name'] .'][instagram]" id="' . $field['id'] . '" value="'; if ( isset($this_value['instagram']) ) echo esc_attr($this_value['instagram']); echo '" size="30" />';
										...
										echo '<label>Pinterest: </label><input type="text" name="' . $field['id'] . '['.$i.']['. $this_field['name'] .'][pinterest]" id="' . $field['id'] . '" value="'; if ( isset($this_value['pinterest']) ) echo esc_attr($this_value['pinterest']); echo '" size="30" />';
										...
										echo '<label>LinkedIn: </label><input type="text" name="' . $field['id'] . '['.$i.']['. $this_field['name'] .'][linkedin]" id="' . $field['id'] . '" value="'; if ( isset($this_value['linkedin']) ) echo esc_attr($this_value['linkedin']); echo '" size="30" />';
							...
							foreach ( $field['repeat-fields'] as $this_field => $type ) {
								...
								switch ( $type ) {
									...
									case 'social':
										...
										echo '<label>Instagram: </label><input type="text" name="' . $field['id'] . '['.$i.']['. $this_field .'][instagram]" id="' . $field['id'] . '" value="'; if ( isset($this_value['instagram']) ) echo esc_attr($this_value['instagram']); echo '" size="30" />';
										...
										echo '<label>Pinterest: </label><input type="text" name="' . $field['id'] . '['.$i.']['. $this_field .'][pinterest]" id="' . $field['id'] . '" value="'; if ( isset($this_value['pinterest']) ) echo esc_attr($this_value['pinterest']); echo '" size="30" />';
										...
										echo '<label>LinkedIn: </label><input type="text" name="' . $field['id'] . '['.$i.']['. $this_field .'][linkedin]" id="' . $field['id'] . '" value="'; if ( isset($this_value['linkedin']) ) echo esc_attr($this_value['linkedin']); echo '" size="30" />';
			...
			case 'portfolio-fields-input':
				...
				foreach ($portfolio_fields as $portfolio_field) {
					...
					$this_value = is_array($meta) && isset($meta[$portfolio_field['slug']]) ? esc_attr($meta[$portfolio_field['slug']]['value']) : '';
...
function quadro_save_custom_data( $post_id ) {
	...
	// for modules specific custom fields
