...
function quadro_options_validate( $input ) {
	...
	do_action( 'at_theme_options_updated_' . $submittab );
...
function quadro_setting_callback( $option ) {
	...
	// Output upload form field markup
		...
		<p>Once you upload the file, click "Choose File".</p>
		<?php
	}
	...
	// Output NEW dummy content import options markup
		...
		echo '<div class="loader-icon" style="display: none;"><i class="fa fa-spinner fa-spin"></i> ' . esc_html__('This may take a while. Please, don\'t refresh your browser till you get a nice message saying it went ok. ;)', 'quadro') . '</div>';
		echo '<a href="#" id="quadro_dcontentx_button" class="button button-primary" title="' . esc_html__('Open Importer', 'quadro') . '">' . esc_html__('Open Importer', 'quadro') . '</a>';
	...
	// Output user check markup
		...
		echo '<a href="#" id="quadro_user_check" class="button" title="' . esc_html__('Check Now', 'quadro') . '">' . esc_html__('Check Now & Refresh Notices', 'quadro') . '</a>';
	}
...
function quadro_ajax_callback()
{
	...
	elseif ( $call_type == 'typekit_activate' ) {
		...
		$check_url = 'https://artisanthemes.io/api/typekit-license-check';
	...
	elseif ( $call_type == 'user_check' ) {
		...
		global $theme_slug;
		// double set for Binder PRO
		$theme_slug = $theme_slug == 'binder' ? 'binderpro' : $theme_slug;
		...
		if ( !is_wp_error($raw_response) && ($raw_response['response']['code'] == 200) ) {
			...
			// refresh notice transients
			at_refresh_license_checks();
			// Print the proper response now
			if ( $response == 'ok' ) {
				echo esc_html__('Your license was sucesfully verified.', 'quadro');
			} else {
				echo $response;
			}
		} else if ( is_wp_error($raw_response) ) {
			// If there was a WP error, print it
			echo $raw_response->get_error_message();
		}
...
function qi_dcontent_lightbox() {
	...
	<div id="dcontent-selector" style="display: none;">
		<div class="dcontent-wrapper">
			<h2><?php esc_html_e('Browse Available Ready Made Sites', 'quadro'); ?></h2>
...
function quadro_ajax_dcontent() {
	...
	if ( $call_type == 'dcontent_details' ) {
		...
		echo '</p>';
		if ( isset( $dcontent_packs[$pack]['images'] ) && $dcontent_packs[$pack]['images'] == true ) {
			echo '<small>' . esc_html__( '* Images will be imported', 'quadro' ) . '</small>';
		} else {
			echo '<small>' . esc_html__( '* Images might not be included', 'quadro' ) . '</small>';
		}
		echo '<div>';
		...
		echo '<div class="dcontent-import-process"></div>';
		echo '</div>';
		echo '</div>';
