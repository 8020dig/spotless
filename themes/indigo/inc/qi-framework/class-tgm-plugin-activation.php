    ...
    class TGM_Plugin_Activation {
        ...
        public function install_plugins_page() {
            ...
            <div class="tgmpa wrap">
                ...
                ?>
                <?php // We don't need to show the filtered views right now.
                // $plugin_table->views(); ?>
        ...
        public function get_tgmpa_url() {
            ...
            if ( ! isset( $url ) ) {
                ...
                $url = add_query_arg(
                    array(
                        // We define here a visible tab for the Getting Started page
                        'page' => urlencode( $this->menu ) . '&current-tab=plugins',
                    ),
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            foreach ( $plugins[ $this->view_context ] as $slug => $plugin ) {
                ...
                $table_data[ $i ]['description']       = '<p>' . $plugin['description'] . '</p>';
        ...
        protected function get_plugin_advise_type_text( $required ) {
            if ( true === $required ) {
                return wp_kses_post( __( '<span class="tgm-required">Required</span>', 'tgmpa' ) );
            }
            return wp_kses_post( __( '<span class="tgm-recommended">Recommended</span>', 'tgmpa' ) );
        }
        /**
        ...
        protected function get_plugin_source_type_text( $type ) {
            ...
            switch ( $type ) {
                ...
                case 'external':
                    ...
                    break;
                case 'bundled':
                    $string = esc_html__( 'Theme Package', 'tgmpa' );
                    break;
            }
            return $string;
        }
        ...
        protected function get_plugin_status_text( $slug ) {
            if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
                return wp_kses_post( __( '<span class="tgm-not-installed">Not installed</span>', 'tgmpa' ) );
            }
            if ( ! $this->tgmpa->is_plugin_active( $slug ) ) {
                $install_status = wp_kses_post( __( '<span class="tgm-installed-but">Installed but not activated</span>', 'tgmpa' ) );
            } else {
                $install_status = wp_kses_post( __( '<span class="tgm-active">Active</span>', 'tgmpa' ) );
            }
            $update_status = '';
            if ( $this->tgmpa->does_plugin_require_update( $slug ) && false === $this->tgmpa->does_plugin_have_update( $slug ) ) {
                $update_status = wp_kses_post( __( '<span class="tgm-update-but">Required update not available</span>', 'tgmpa' ) );
            } elseif ( $this->tgmpa->does_plugin_require_update( $slug ) ) {
                $update_status = wp_kses_post( __( '<span class="tgm-update-required">Requires update</span>', 'tgmpa' ) );
            } elseif ( false !== $this->tgmpa->does_plugin_have_update( $slug ) ) {
                $update_status = wp_kses_post( __( '<span class="tgm-update-recommended">Update recommended</span>', 'tgmpa' ) );
            }
            if ( '' === $update_status ) {
        ...
        public function column_plugin( $item ) {
            return '<div class="tgm-plugin-desc">' . sprintf(
                '%1$s %2$s %3$s',
                $item['plugin'],
                $item['description'],
        ...
        protected function get_row_actions( $item ) {
            ...
            if ( ! $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
                $actions['install'] = _x( 'Install %2$s from ' . $item['source'], 'tgmpa' );
            } else {
                // Display the 'Update' action link if an update is available and WP complies with plugin minimum.
                if ( false !== $this->tgmpa->does_plugin_have_update( $item['slug'] ) && $this->tgmpa->can_plugin_update( $item['slug'] ) ) {
                    $actions['update'] = _x( 'Update %2$s', '%2$s', 'tgmpa' );
                }
                // Display the 'Activate' action link, but only if the plugin meets the minimum version.
                if ( $this->tgmpa->can_plugin_activate( $item['slug'] ) ) {
                    $actions['activate'] = _x( 'Activate %2$s', '%2$s', 'tgmpa' );
                }
            }
