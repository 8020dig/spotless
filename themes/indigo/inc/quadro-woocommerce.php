...
/**
 * Moves "Add to Cart" button over the thumbnail in loop
 */
function qi_move_loop_button(){
	remove_action( 'woocommerce_after_shop_loop_item', 'woocommerce_template_loop_add_to_cart', 10 );
	add_action( 'woocommerce_before_shop_loop_item_title', 'woocommerce_template_loop_add_to_cart', 15 );
}
add_action( 'init','qi_move_loop_button' );
	...
	?><li>
		<div class="header-cart closed-cart">
			<?php if( !$woocommerce->cart->cart_contents_count ) { ?>
				<a class="header-cart-link-mobile" href="<?php echo esc_url( get_permalink( get_option('woocommerce_cart_page_id') ) ); ?>">
				...
				<div class="cart-contents">
					...
					<div class="cart-inner clear">
						<p class="cart-empty-msg"><?php esc_html_e('You have no items in your cart.', 'indigo') ?></p>
					</div>
				</div>
			<?php } else { ?>
				<a class="header-cart-link-mobile" href="<?php echo esc_url( get_permalink( get_option('woocommerce_cart_page_id') ) ); ?>">
					<i class="fa fa-shopping-bag"></i><span class="header-cart-qy"><?php echo esc_attr( $woocommerce->cart->cart_contents_count ); ?></span>
				</a>
				<span class="header-cart-link header-cart-link-active">
					<i class="fa fa-shopping-bag"></i><span class="header-cart-qy"><?php echo esc_attr( $woocommerce->cart->cart_contents_count ); ?></span>
				</span>
				<div class="cart-contents">
					<span class="close-header-cart"></span>
					<div class="cart-inner clear">
						<ul class="clear">
							<?php foreach( $woocommerce->cart->cart_contents as $cart_item ): ?>
								<li class="cart-content">
									...
									<?php echo get_the_post_thumbnail( $cart_item['product_id'], 'quadro-sq-thumb' ); ?>
									<div class="product-details">
										<span class="product-title"><?php echo esc_attr( $cart_item['data']->post->post_title ); ?></span>
										...
										<?php // output description if only one product
										if ( count($woocommerce->cart->cart_contents) === 1 ) { ?>
											<p class="product-description"><?php echo quadro_excerpt( get_post_field('post_content', $cart_item['product_id']), 25, '', $tags = '' ); ?></p>
										<?php } ?>
									</div>
									</a>
								</li>
						...
						<section class="cart-data">
							<h1><?php echo esc_html__('Your Cart', 'indigo'); ?></h1>
							<div class="cart-total">
								<p><?php echo esc_html__('Cart Subtotal ', 'indigo') . '<span>' . $woocommerce->cart->get_cart_total() . '</span>'; ?></p>
							</div>
							<div class="cart-actions">
								<div class="cart-link"><a href="<?php echo esc_url( get_permalink( get_option('woocommerce_cart_page_id') ) ); ?>" title="<?php esc_html_e('View Cart', 'indigo'); ?>" class="cart-link-a"><?php esc_html_e('View Cart', 'indigo'); ?></a></div>
								<div class="checkout-link"><a href="<?php echo esc_url( get_permalink( get_option('woocommerce_checkout_page_id') ) ); ?>" title="<?php esc_html_e('Checkout', 'indigo'); ?>" class="qbtn"><?php esc_html_e('Checkout', 'indigo'); ?></a></div>
							</div>
						</section>
					</div>
				</div>
