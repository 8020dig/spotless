<?php
/**
 * artisanthemes functions and definitions
 *
 * @package quadro
 */
...
/**
 * Deactivates and deletes old Mods type plugin.
 */
function at_deactivate_modstype_plugin() {
	deactivate_plugins( '/quadro-mods/quadro-mods.php' );
}
add_action( 'admin_init', 'at_deactivate_modstype_plugin' );
...
function quadro_setup() {
	 ...
	 */
	load_theme_textdomain( 'indigo', $template_directory . '/languages' );
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary Menu', 'indigo' ),
		'user' => esc_html__( 'User/Login Menu', 'indigo' ),
	) );
...
if ( is_admin() ) {
	...
	// Include RMS Available Packs
	require( $template_directory . '/inc/dcontent_available.php' );
...
if ( is_admin() ) { 
	...
	$theme_slug = 'indigo';
	// Theme's Docs URL
	$qi_docs_url = '//artisanthemes.io/docs/Indigo_Documentation.html';
	// Theme Support URL
	$qi_support_url = 'https://artisanthemes/support-center';
}


/**
 * Including Artisan Themes Framework
 */
...
/**
 * Including Artisan Custom Post Types
 */
...
function quadro_register_required_plugins() {
	...
	$plugins = array(
		...
		// Artisan Themes Portfolio
		array(
			'name'     				=> 'Artisan Themes - Portfolio Type',
			...
			'description'			=> esc_html__('This plugin enables the Portfolio custom post type that we use to create Portoflio Items.', 'indigo'),
		),
		...
		array(
			'name'     				=> 'Artisan Pages',
			'slug'     				=> 'artisan-pages',
			'source'   				=> 'https://artisanthemes.io/at-plugins/repo/artisan-pages.zip',
			'required' 				=> false,
			'version' 				=> '1.0.2',
			'force_activation' 		=> false,
			'force_deactivation' 	=> true,
			'description'			=> esc_html__('Artisan Pages lets you automagically create modular pages on your site to make your building process a lot faster. Browse the catalog, and start from the one that suites you the most.', 'indigo'),
		),

		// Artisan Themes Shortcodes
		array(
			'name'     				=> 'Artisan Themes - Shortcodes',
			'slug'     				=> 'quadro-shortcodes',
			'source'   				=> get_stylesheet_directory() . '/inc/plugins/quadro-shortcodes.zip',
			'required' 				=> false,
			'version' 				=> '1.0.1',
			'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation' 	=> false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
			'description'			=> esc_html__('This plugin enables Artisan Themes Shortcodes. For now, it only contains the fabulous Typed.js effect. Install it if you want to create animated-typed sentences.', 'indigo'),
		),
		...
		array(
			...
			'required'				=> false,
			'version'				=> '1.5.3',
			'description'			=> esc_html__('Build page layouts using the widgets you know and love using this simple drag and drop page builder.', 'indigo'),
		),
		...
		array(
			...
			'required'				=> false,
			'version'				=> '1.4.2',
			'description'			=> esc_html__('A collection of widgets to be used together with Page Builder plugin.', 'indigo'),
		),
		...
		array(
			...
			'required'				=> false,
			'version'				=> '1.5.7',
			'description'			=> esc_html__('Responsive Lightbox allows users to view larger versions of images and galleries in a lightbox (overlay) effect optimized for mobile devices.', 'indigo'),
		),
		...
		array(
			...
			'required'				=> false,
			'version'				=> '1.0.1',
			'description'			=> esc_html__('Mega Menus plugin lets you create visual enhanced submenus with "post style" content.', 'indigo'),
		),
	...
	$config = array(
		...
		'strings'      		=> array(
			'page_title'                       			=> esc_html__( 'Required Plugins Area (Install/Update)', 'indigo' ),
			'menu_title'                       			=> esc_html__( 'Install Required Plugins', 'indigo' ),
			'installing'                       			=> esc_html__( 'Installing Plugin: %s', 'indigo' ),
			'oops'                             			=> esc_html__( 'Something went wrong with the plugin API.', 'indigo' ),
			'notice_can_install_required'     			=> _n_noop( wp_get_theme() . ' requires the following plugin: %1$s.', wp_get_theme() . ' requires the following plugins: %1$s.', 'indigo' ),
			'notice_can_install_recommended'			=> _n_noop( wp_get_theme() . ' recommends the following plugin: %1$s.', wp_get_theme() . ' recommends the following plugins: %1$s.', 'indigo' ),
			'notice_cannot_install'  					=> _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'indigo' ),
			'notice_can_activate_required'    			=> _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'indigo' ),
			'notice_can_activate_recommended'			=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'indigo' ),
			'notice_cannot_activate' 					=> _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'indigo' ),
			...
			'notice_cannot_update' 						=> _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'indigo' ),
			'install_link' 					  			=> _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'indigo' ),
			'activate_link' 				  			=> _n_noop( 'Activate installed plugin', 'Activate installed plugins', 'indigo' ),
			'return'                           			=> esc_html__( 'Return to Required Plugins Installer', 'indigo' ),
			'plugin_activated'                 			=> esc_html__( 'Plugin activated successfully.', 'indigo' ),
			'complete' 									=> esc_html__( 'All plugins installed and activated successfully. %s', 'indigo' ), // %1$s = dashboard link
